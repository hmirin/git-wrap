...
pub fn run(_args: &[String]) -> Result<()> {
    ...
    println!("    blockAmendPushed:  block --amend on pushed commits (use --yes)");
    println!();
    println!("  {}", "auto:".bold());
    ...
    println!("    setUpstreamOnPull:  auto set-upstream on first pull");
    println!("    pruneOnFetch:       auto --prune on fetch (clean stale branches)");
    println!();
    println!("  {}", "commit:".bold());
